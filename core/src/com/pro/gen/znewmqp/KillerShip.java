...
public class KillerShip extends Group {
    ...
    public KillerShip(AbsPopup popup){
        this.popup = popup;
        setVisible(false);
        shipHolder = new TintedImage(Pic.Pixel, Tint.MEDIUM_GRAY);
        shipHolder.setSize(7,25);
        shipHolder.setPosition(1040, 410);
        addActor(shipHolder);
        shipGun = new TintedImage(Pic.Ship_Gun);
        shipGun.setSize(80, 40);
        shipGun.setPosition(995, 430);
        addActor(shipGun);
        ship = new TintedImage(Pic.Ship);
        ship.setSize(100, 50);
        ship.setPosition(1020, 370);
        addActor(ship);


        beam = new TintedImage(Pic.Curve_rectangle, Tint.BLAST_RED);
        beam.setSize(10, 20);
        beam.setPosition(988, 440);
        beam.setVisible(false);
        addActor(beam);

        beamTip = new TintedImage(Pic.Circle_Large, Tint.BLAST_RED);
        beamTip.setSize(1, 1);
